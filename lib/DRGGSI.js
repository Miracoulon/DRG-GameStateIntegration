...
class DRGGSI extends EventEmitter {
    ...
    digest(data) {
        ...
        if (!raw)
            throw new TypeError('Raw data was null or undefined');
        if (!raw.Token)
            throw new SyntaxError('Data had no Token');
        if (!raw.Token.Name === undefined)
            throw new SyntaxError('Token Name is missing');
        if (!raw.Token.Value === undefined)
            throw new SyntaxError('Token Value is missing');
        if (!this.checkAuth(raw.Token.Name, raw.Token.Value)) {
            this.emit('info', 'Invalid auth');
            return false;
        }
        ;
        this.emit('raw', raw);
        if (!raw.Type)
            throw new SyntaxError('Data had no assigned type');
        if (!raw.Data)
            throw new SyntaxError('Data was empty');
        if (!this._messageHandlers.has(raw.Type)) {
            this.emit('info', `No message handler for "${raw.Type}"`);
            return false;
        }
        ;
        try {

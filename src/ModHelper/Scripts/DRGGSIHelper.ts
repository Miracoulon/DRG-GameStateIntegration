const sockets = new Map<string, DRGGSISocket>();
 ...
 */
function writeError(message: string, socketID: string) {
 ...
 */
function sendDirect(socketID: string, messageType: string, message: string | object) {
    if (!socketID) {
 ...
 * @param {string | object} message The actual content of the message.
 */
function sendMulti(socketIDs: Array<string>, messageType: string, message: string | object) {
...
function handleSocketOpen(socketID: string) {
    if (!sockets.has(socketID)) {
...
function handleSocketError(socketID: string, error) {
    writeError(error, socketID);
}

function handleSocketClose(socketID: string, closeEvent) {
 ...
 */
function disconnect(socketID: string, reason: string = 'Client initiated disconnect') {
    if (sockets.has(socketID)) {
        const socket = sockets.get(socketID);
